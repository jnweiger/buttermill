...
*
*****************************************************************************/
void ADC_periphery(char *retval)
{
    int ADCresult = 0;
    //int ADCresult_temp = 0;
    //int Temp_int;
    unsigned char Temp; // bit operations are unsigned
    unsigned char i = 0;
    char TL;
    char TH;
    unsigned int convertme = 0;
    //char VL;
    ...
    if( ADMUX == TEMPERATURE_SENSOR )
    {
        if(degree == CELSIUS)
        {
            if(ADCresult > 810)         // If it's a negative temperature
            {    
    ...
    else if( ADMUX == LIGHT_SENSOR )
    {
        ...
        convertme = CHAR2BCD3(buf);
        
        retval[0] = ((convertme<<8)&0x0f)+'0';
        retval[1] = ((convertme<<4)&0x0f)+'0';
        retval[2] = (convertme&0x0f)+'0';
        retval[3] = '\0';
        return;
    }
}
