...
#                         mk_fuse_default() added. direct assignment 'wrfuse lfuse=0xe4' implemented.
# 2008-09-25, jw, v0.9a -- test $parport, and do not fail on macros.
# 2008-09-25, jw, v0.9b -- all mhz values from the baud rate tables in doc2545 added.
#                          Fallback to p version of the cpu if the one without has no FUSE_* defs.
#                          Option -r for running programmer unter sudo added.
# 2008-09-28, jw, v0.9c -- fixed newly introduced $parport sanity check.
# 2008-09-29, jw, v0.9d -- tuned verboseness, made all internal oscillators available as clock=i...hz
# 2009-04-26, jw, V0.9e -- support for mega16 added.
...
while (defined (my $arg = shift))
  {
    ...
    elsif ($arg eq '-P')	{ $parport = shift; $serport = ''; }
    elsif ($arg eq '-p')	{ $cpu = shift; }
    elsif ($arg eq '-S')	{ $serport = shift; $parport = ''; }
...
if ($programmer =~ m{dude}i)
  {
    ...
    $parport = '' if lc $dongle_type eq 'usbasp';
    die "$parport: $!\n" if length $parport and !$noop and ! -c $parport;	# test if the char-device exists.

    $cmd .= "$programmer -c $dongle_type -p AT$cpu";
    $cmd .= " -P $parport" if length $parport;
    ...
    if ($opcode =~ m{^up\w+ee})
      ...
      {
        # usbasp complains '-E noreset' not supported.
        $cmd .= qq{ -E noreset} unless lc $dongle_type eq 'usbasp';
      }
...
Valid options are:
 ...
 -P parport     	Specify parallel-port with dongle. Default: $parport
 -S serport     	Specify serial-port with dongle. Default: $serport
  ...
  while (defined (my $line = <IN>))
    {
	  ...
	  # its a C expression, it should work in perl too.
	  my $val = eval $cval;	
